...
const study = require('./study-session-backend');

describe('convertTo12HourFormat', () => {
    test('Converts morning times (AM)', () => {
        expect(study.convertTo12HourFormat('08:30')).toBe('8:30 AM');
    });

    test('Converts afternoon times (PM)', () => {
        expect(study.convertTo12HourFormat('15:45')).toBe('3:45 PM');
    });

    test('Converts midnight (12:00 AM)', () => {
        expect(study.convertTo12HourFormat('00:00')).toBe('12:00 AM');
    });

    test('Converts noon (12:00 PM)', () => {
        expect(study.convertTo12HourFormat('12:00')).toBe('12:00 PM');
    });

    test('Converts times with single-digit minutes', () => {
        expect(study.convertTo12HourFormat('09:05')).toBe('9:05 AM');
    });

    test('Converts times with leading zeros in hours', () => {
        expect(study.convertTo12HourFormat('03:20')).toBe('3:20 AM');
    });

    test('Converts times with leading zeros in minutes', () => {
        expect(study.convertTo12HourFormat('14:09')).toBe('2:09 PM');
    });
});
...
//         expect(study.formatDate('2024-04-25')).toBe('April 25, 2024');
//     });

//     test('Formats a date with leading zeros in the day or month', () => {
//         expect(study.formatDate('2024-04-05')).toBe('April 05, 2024');
//     });

//     test('Formats a date with different locales', () => {
//         // Example of how you might test with a different locale
//         const formattedDate = study.formatDate('2024-04-25');
...
//         expect(study.formatDate('2024-04-30')).toBe('April 30, 2024');
//     });

//     test('Formats a date at the end of the year', () => {
//         // Example of testing a date at the end of the year
//         expect(study.formatDate('2024-12-31')).toBe('December 31, 2024');
//     });
// });
...
describe('setDifference', () => {
    test('Difference between sets with different elements', () => {
        ...
        expect(study.setDifference(setA, setB)).toEqual(new Set([1, 2, 3]));
    });

    test('Difference between sets with some overlapping elements', () => {
        const setA = new Set([1, 2, 3, 4]);
        const setB = new Set([3, 4, 5, 6]);
        expect(study.setDifference(setA, setB)).toEqual(new Set([1, 2]));
    });

    test('Difference between sets where one set is a subset of the other', () => {
        const setA = new Set([1, 2, 3]);
        const setB = new Set([2]);
        expect(study.setDifference(setA, setB)).toEqual(new Set([1, 3]));
    });

    test('Difference between two empty sets', () => {
        const setA = new Set();
        const setB = new Set();
        expect(study.setDifference(setA, setB)).toEqual(new Set());
    });
    ...
    test('Difference between two sets with non-primitive elements', () => {
        ...
        expect(study.setDifference(setA, setB)).toEqual(new Set([obj1, obj3]));
    });
...
describe('sendEmailOnUpdate', () => {
    test('Sends email with correct session info', () => {
        ...
        // Call the function
        study.sendEmailOnUpdate('participant@example.com', sessionInfo);
...
describe('sendEmailOnDelete', () => {
    test('Sends email on delete - Success', async () => {
        ...
        await study.sendEmailOnDelete('participant@example.com', sessionInfo);
